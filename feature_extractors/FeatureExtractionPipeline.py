...
class FeatureExtractionPipeline:
    ...
    def export_for_experiments(self, 
                         train_df, 
                         val_df, 
                         output_dir="data/processed/"):
        """
        Export processed data for experiment notebooks
        Compatible with your existing DataLoader class
        
        Args:
            train_df: Training dataframe (with labels column)
            val_df: Validation dataframe (with labels column)  
            output_dir: Where to save the data
        """
        if not self.results:
            print("❌ No results to export. Run extract_features() first.")
            return
        
        print(f"💾 Exporting data for experiments...")
        
        # Create output directory
        os.makedirs(output_dir, exist_ok=True)
        
        # Export for each extractor (using your DataLoader's expected format)
        for extractor_name, result in self.results.items():
            print(f"📁 Exporting {extractor_name}...")
            
            # Save features (using your DataLoader's naming convention)
            with open(f"{output_dir}/X_train.pkl", 'wb') as f:
                pickle.dump(result['train']['features'], f)
            
            with open(f"{output_dir}/X_val.pkl", 'wb') as f:
                pickle.dump(result['val']['features'], f)
            
            print(f"   ✅ Features saved: {result['train']['shape']} train, {result['val']['shape']} val")
        
        # Save labels (using your DataLoader's naming convention)
        with open(f"{output_dir}/y_train.pkl", 'wb') as f:
            pickle.dump(train_df['labels'].values, f)
        
        with open(f"{output_dir}/y_val.pkl", 'wb') as f:
            pickle.dump(val_df['labels'].values, f)
        
        print(f"   ✅ Labels saved: {len(train_df)} train, {len(val_df)} val")
        ...
        print(f"📄 Use your existing DataLoader in experiment notebooks")
